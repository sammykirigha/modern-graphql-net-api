...
public class RolePermissionService : IRolePermissionService
{
    ...
    public IQueryable<RolePermission> GetList()
    {
        return _repository.GetList().AsNoTracking();
    }

    public IQueryable<RolePermissionPaged> GetPagedList()
    {
        return (
            from u in _repository.GetList()
            select new RolePermissionPaged
            {
                RolePermission = u
            }
        ).AsNoTracking();
    ...
    // MUTATIONS
    public async Task<RolePermission> AddAsync(RolePermission input, EntityLogInfo logInfo)
    {
    ...
    public async Task<RolePermission> UpdateAsync(RolePermission input, EntityLogInfo logInfo, RolePermission? oldEntity = null)
    {
        using var trans = await _repository.BeginTransactionAsync();

        oldEntity ??= await _repository.GetByIdAsync(input.Id)
                      ?? throw new AppException($"{nameof(RolePermission)} not found.", ValidationCode.MissingRequirementEntity);
