...
{
  private readonly ISubscriptionPaymentsRepository _repository;
    private readonly IEntityLogService _log;

    public SubscriptionPaymentsService(
        ISubscriptionPaymentsRepository repository,
    ...
    public IQueryable<SubscriptionPayments> GetList()
    {
        return _repository.GetList().AsNoTracking();
    }

    public IQueryable<SubscriptionPaymentsPaged> GetPagedList()
    {
        return (
            from u in _repository.GetList()
            select new SubscriptionPaymentsPaged
            {
                SubscriptionPayments = u
            }
        ).AsNoTracking();
    }
    ...
    // MUTATIONS
    public async Task<SubscriptionPayments> AddAsync(SubscriptionPayments input, EntityLogInfo logInfo)
    {
    ...
    public async Task<SubscriptionPayments> UpdateAsync(SubscriptionPayments input, EntityLogInfo logInfo, SubscriptionPayments? oldEntity = null)
    {
	    using var trans = await _repository.BeginTransactionAsync();
