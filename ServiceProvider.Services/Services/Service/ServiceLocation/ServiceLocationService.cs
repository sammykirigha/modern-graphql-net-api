...
public class ServiceLocationService : IServiceLocationService
{
    ...
    public IQueryable<ServiceLocation> GetList()
    {
        return _repository.GetList().AsNoTracking();
    }

    public IQueryable<ServiceLocationPaged> GetPagedList()
    {
        return (
            from u in _repository.GetList()
            select new ServiceLocationPaged
            {
                ServiceLocation = u
            }
        ).AsNoTracking();
    ...
    // MUTATIONS
    public async Task<ServiceLocation> AddAsync(ServiceLocation input, EntityLogInfo logInfo)
    {
    ...
    public async Task<ServiceLocation> UpdateAsync(ServiceLocation input, EntityLogInfo logInfo, ServiceLocation? oldEntity = null)
    {
	    using var trans = await _repository.BeginTransactionAsync();
